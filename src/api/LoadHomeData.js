export async function loadHomeData(setLoading) {
    setLoading(true);

    try {
        const response = await fetch("http://localhost:8080/hireup_backend/LoadHomeData", {
            method: "GET",
            headers: {
        ...
        if (response.ok) {
            ...
            if (json.status) {
                ...
                } else if (json.message === "NHRECOMMENDED") {
